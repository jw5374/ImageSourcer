...
bool checkRegKey(string handle, string path) {
	return false;
}

void setRegKey(HKEY handle, wstring path) {
	wstring strManPath = L"D:\\CodeProjects\\Chromeext\\ImageSourcer\\ImageSourcerHost\\manifest.json\0"; // Path should be changed to whereever the host manifest.json is
	HKEY key;
